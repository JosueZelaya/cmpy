...
insert into categoria (nombre,descripcion,modification_time,creation_time,categoria_padre_id,sis_activo,version) values ('Servicios','...',current_date,current_date,null,true,1);--226

update categoria set categoria_padre_id = (select id from categoria where nombre='Servicios')  where id>=27 and id<=57;

update categoria set nombre = replace(nombre,'Servicio de ','') where nombre like '%Servicio de%';

update categoria set nombre = 'Construccion' where id = 30;

update categoria set nombre = 'Impresion' where id = 32;
